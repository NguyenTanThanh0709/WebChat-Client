...
const notificationApi = {
    getNotifications(phone: string, query: GetNotificationsQuery) {
        return http1.get<Notification[]>(`/api/notification/get-notifications/${phone}`, { params: query })
    },

    createNotification(body: CreateNotificationInput) {
        return http1.post<Notification>('/api/notification/send', body)
    },

    updateNotification(id: string, body: Notification) {
        return http1.patch<Notification>(`/api/notification/send/${id}`, body)
    }
}
